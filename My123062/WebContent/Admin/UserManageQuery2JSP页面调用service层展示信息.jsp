	...
	<!-- 测试begin -->
						...
						if(null!=list && list.size()>0){
							for(User user:list){
								...
								<input  type="checkbox" name="checkbox" value="${userKK.id }"></td>
								<td width="98" bordercolor="#FFFFFF" class="text_cray1">${userKK.userName}</td>
								<td width="80" bordercolor="#FFFFFF" class="text_cray1">${userKK.sex=="M"?"男":"女"}</td>
								<td width="132" bordercolor="#FFFFFF" class="text_cray1">
								${userKK.certType.id==1?"二代身份证":userKK.certType.id==2?"港澳通行证":userKK.certType.id==3?"台湾通行证":"护照"}
								</td>
								<td width="247" bordercolor="#FFFFFF" class="text_cray1">${userKK.cert}</td>
								<td width="82" bordercolor="#FFFFFF" class="text_cray1">
								${userKK.userType.id==1?"成人":userKK.userType.id==2?"儿童":userKK.userType.id==3?"学生":"残疾军人、伤残人民警察"}
								</td>
