...
<%-- <%
if(null!=request.getParameter("action")&&request.getParameter("action").equals("queryBack")){
...
<script language="javascript">
	...
	function deleteMore(){
	 ...
	 function queryAll(type){
			...
			form.submit(); */
			$.ajax({
				url:"QueryServlet?action=queryAjax",
				type:"post",
				data:$("#form1").serialize(),
...
<script language="javascript">
	...
	//实现动态的tr新增
		function doInnerHTML(jsonArray){
		   //console.log("记录数="+jsonArray[0]);
		   //console.log("json="+jsonArray[1]);
 			 var $dom=$("#data");
			$dom.empty();
			
			 var $page=$("#page");
			$page.empty();
			for(var i=0;i<jsonArray[0];i++){
				var allStr="<a href='javascript:void(0);' onclick='gotoPage("+(i+1)+");'>"+(i+1)+"</a>";
				$page.append(allStr);
			} 
			
			jsonArray=jsonArray[1];
			for(var i=0;i<jsonArray.length;i++){
				var userK=jsonArray[i];
				var bgc=(i+1)%2==0?"#FFFFFF":"pink";
				var realName=userK.realName==undefined?"":userK.realName;
				var sex=userK.sex=="M"?"男":"女";
				var allStr="<tr align='center' bgcolor='"+bgc+"' >"+
				"<td bordercolor='#FFFFFF'  class='text_cray1'>"+(i+1)+
				"<input  type='checkbox' name='checkbox'  onclick='doSelOne(this);' value='"+userK.id+"'></td>"+
				"<td width='98' bordercolor='#FFFFFF'  bgcolor='#FF69B4' class='text_cray1'>"+(i+1)+"</td>"+
				"<td width='98' bordercolor='#FFFFFF'  bgcolor='#FF69B4'  class='text_cray1'>"+realName+"</td>"+
				"<td width='80' bordercolor='#FFFFFF'  class='text_cray1'>"+sex+"</td>"+
				...
				"<a href='QueryServlet?action="+userK.id+"'  target='_blank' class='text_red'>编辑"+userK.id+"</a></td></tr>";
				$dom.append(allStr);
			} 
	}
...
<input type="hidden" name="pageCnt"  id="pageCnt" value='${empty sessionScope.pageCnt?"0":pageCnt}'>
				...
				<td width="835" background="../images/wb_01 (3).jpg"><table
						...
						<tr>
							...
							<td width="11%" align="left" class="text_cray1"><label>
									<input name="realName"  value="${pageUserParam.realName }" type="text" class="text_cray" style="width: 80px">
							</label></td>
...
<tbody id="data">
					...
					<%-- 	<c:forEach items="${userKK}" var="userK" varStatus="status"  >
	...
	<%--  <c:if test="${empty pageCnt==false }">
		...
		<a href="javascript:void(0);" onclick="nextPage();">&gt;&gt;</a>	
							...
							<td width="154" align="right" class="text_cray1"
								...
								<!--  -->
								<select  name="pageSize"  onclick="queryAll();">
